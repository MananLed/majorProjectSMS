...
func handler(ctx context.Context, event events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	now := time.Now()
	if err := noticeService.IssueNotice(req.Content, now.Month(), now.Year()); err != nil {
		return response.ErrorResponse(http.StatusInternalServerError, "Failed to issue notice", 1006), nil
	}

	return response.SuccessResponse(nil, "Notice issued successfully!!", http.StatusOK), nil
}
